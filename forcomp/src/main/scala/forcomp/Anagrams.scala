...
object Anagrams {
  ...
  /** Converts the word into its character occurence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (w groupBy ((c: Char) => c.toLower)).toList map (p => p._1 -> p._2.size) sortWith ((o1, o2) => o1._1 < o2._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      for (
        ...
        it <- ((1 to occ) map (i => char -> i)).toList;
        res <- List(List()) ::: List(List(it)) ::: (combinations(occurrences.tail) map (l => it :: l))
        if res.groupBy(c => c._1).forall(e => e._2.length == 1)
      ) yield res
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x.foldLeft(List[(Char, Int)]())((r, c) => (c._1, c._2 - yMap(c._1)) :: r) filter (p => p._2 > 0) sortBy (p => p._1)
  }

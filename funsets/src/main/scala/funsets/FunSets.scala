...
object FunSets {
   ...
   */
  def diff(s: Set, t: Set): Set = (e:Int) => contains(s, e) && !contains(intersect(s, t), e)

  /**
   * Returns the subset of `s` for which `p` holds.
   */
  def filter(s: Set, p: Int => Boolean): Set = (e: Int) => contains(intersect(s, p), e)
   ...
   */
  def map(s: Set, f: Int => Int): Set = (e:Int) => exists(s, (x: Int) => e == f(x))
